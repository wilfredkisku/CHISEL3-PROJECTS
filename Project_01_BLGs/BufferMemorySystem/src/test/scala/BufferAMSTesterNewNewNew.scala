...
class BufferAMSTesterNewNewNew extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "accumulate zero counts, sort them, and maintain indices, with reset behavior" in {
    test(new BufferAMSNew).withAnnotations(Seq(WriteVcdAnnotation)) { c =>
      ...
      c.clock.step(3)       // Step clock multiple times to ensure reset propagates fully
      c.reset.poke(false.B) // Deassert reset signal
      c.clock.step(3)       // Step clock after reset to allow the module to stabilize

      // Give the module a few clock cycles to stabilize after reset
      c.clock.step(3)       // Extra steps to ensure proper reset and no residual state
